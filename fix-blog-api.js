...
console.log('🔧 FIXING: Blog post content via API...');

async function fixBlogContent() {
  try {
    const response = await fetch('http://localhost:5000/api/blog/posts/a15653f3-b42b-4378-b841-8edc95897e44', {
      method: 'PATCH',
      headers: {
    ...
    if (response.ok) {
      ...
      // Verify the fix
      const verifyResponse = await fetch('http://localhost:5000/api/blog/posts/a15653f3-b42b-4378-b841-8edc95897e44');
      const verifyData = await verifyResponse.json();
      
      if (verifyData.content && !verifyData.content.includes('---</p>')) {
        ...
        console.log('🔗 View at: http://localhost:5000/blog/photography-session-7');
      } else {
        console.log('⚠️ WARNING: Verification failed, content may still be malformed');
      }
      
    } else {
      console.error('❌ Failed to update blog post:', await response.text());
    }
