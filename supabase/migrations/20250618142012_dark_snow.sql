...
BEGIN
  ...
  EXECUTE (
    SELECT string_agg('DROP POLICY IF EXISTS "' || policyname || '" ON admin_users;', ' ')
    FROM pg_policies 
...
BEGIN
  ...
  SELECT EXISTS (
    ...
    WHERE tablename = 'admin_users' AND policyname = 'Service role full access'
  ) INTO policy_exists;
  ...
  SELECT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'admin_users' AND policyname = 'Users can read own admin record'
  ) INTO policy_exists;
  
  IF NOT policy_exists THEN
    EXECUTE 'CREATE POLICY "Users can read own admin record"
  ...
  SELECT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'admin_users' AND policyname = 'Users can insert own admin record'
  ) INTO policy_exists;
  
  IF NOT policy_exists THEN
    EXECUTE 'CREATE POLICY "Users can insert own admin record"
  ...
  SELECT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'admin_users' AND policyname = 'Users can update own admin record'
  ) INTO policy_exists;
  
  IF NOT policy_exists THEN
    EXECUTE 'CREATE POLICY "Users can update own admin record"
