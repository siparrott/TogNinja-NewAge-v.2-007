/*
  # Fix Gallery Images Table and Policies

  1. Table Creation
    - Creates gallery_images table if it doesn't exist
    - Adds foreign key constraint to galleries table if it exists
  
  2. Indexes
    - Creates index on gallery_id for faster lookups
    - Creates index on order_index for sorted queries
  
  3. Row Level Security
    - Enables RLS on the table
    - Creates policy for admin users (only if it doesn't exist)
*/
...
BEGIN
  IF NOT EXISTS (
    ...
    EXECUTE 'CREATE POLICY "Admin users can manage gallery images"
      ON gallery_images
      FOR ALL
      TO authenticated
      USING (
